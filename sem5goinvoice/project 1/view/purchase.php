...
(function(){
  async function loadPurchases(){
    var body = document.getElementById('purchaseBody'); if(!body) return;
    try{
      var isLiveServer = /^(127\.0\.0\.1|localhost):55\d{2}$/.test(window.location.host);
      var apiUrl = isLiveServer
        ? 'http://localhost/sem5goinvoice/project%201/api/invoices.php?type=purchase&limit=50'
        : '../api/invoices.php?type=purchase&limit=50';
      ...
      if(!res.ok || data.success !== true){
        var msg = (data && (data.error||data.message)) || 'Failed to load purchase invoices';
        if(/Authentication required/i.test(msg) || res.status === 401){
          ...
          alert('Please login to view purchases. Redirecting to login...'); window.location.href = loginUrl; return;
        }
        throw new Error(msg);
      }
      var list = (data.data && data.data.invoices) ? data.data.invoices : [];
      if(list.length === 0){ body.innerHTML = '<tr><td colspan="6">No purchase invoices yet</td></tr>'; return; }
      body.innerHTML = list.map(function(r){
        ...
        var link = 'print_purchase_invoice.php?id='+ encodeURIComponent(id);
        return '<tr>'+
